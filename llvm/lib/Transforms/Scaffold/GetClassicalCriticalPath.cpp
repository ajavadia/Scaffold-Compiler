...
#define DEBUG_TYPE "GetClassicalCriticalPath"
#include <vector>
#include <limits>
#include "llvm/Pass.h"
...
bool debugGetClassicalCriticalPath = false;
...
namespace {
  ...
  struct GetClassicalCriticalPath : public ModulePass {
    ...
    GetClassicalCriticalPath() : ModulePass(ID) {}
  ...
  }; // End of struct GetClassicalCriticalPath
} // End of anonymous namespace



char GetClassicalCriticalPath::ID = 0;
static RegisterPass<GetClassicalCriticalPath> X("GetClassicalCriticalPath", "Get Critical Path");

void GetClassicalCriticalPath::getFunctionArguments(Function* F)
{
...
bool GetClassicalCriticalPath::backtraceOperand(Value* opd, int opOrIndex)
{
...
void GetClassicalCriticalPath::analyzeAllocInst(Function* F, Instruction* pInst){
...
void GetClassicalCriticalPath::init_critical_path_algo(Function* F){
...
void GetClassicalCriticalPath::print_funcQbits(){
...
void GetClassicalCriticalPath::print_funcQbitsHalf(){
...
void GetClassicalCriticalPath::print_qgate(qGate qg){
...
void GetClassicalCriticalPath::print_critical_info(string func){
...
void GetClassicalCriticalPath::update_critical_info(string currFunc, uint64_t ts, string fname){
...
uint64_t GetClassicalCriticalPath::find_max_funcQbits(){
...
void GetClassicalCriticalPath::memset_funcQbits(uint64_t val){
  for(map<string, map<int,uint64_t> >::iterator mIter = funcQbits.begin(); mIter!=funcQbits.end(); ++mIter){
...
void GetClassicalCriticalPath::memset_funcQbitsHalf(uint64_t val){
  for(map<string, map<int,uint64_t> >::iterator mIter = funcQbitsHalf.begin(); mIter!=funcQbitsHalf.end(); ++mIter){
    for(map<int,uint64_t>::iterator arrIter = (*mIter).second.begin(); arrIter!=(*mIter).second.end();++arrIter)
      (*arrIter).second = val;
  }
...
void GetClassicalCriticalPath::print_scheduled_gate(qGate qg, uint64_t ts){
...
void GetClassicalCriticalPath::print_tableFuncQbits(){
...
void GetClassicalCriticalPath::print_tableFuncQbitsStart(){
...
void GetClassicalCriticalPath::calc_max_parallelism_statistic()
{
...
void GetClassicalCriticalPath::print_tsGates()
{
  for(map<uint64_t, vector<qGate> >::iterator mit = tsGates.begin(); mit!=tsGates.end(); ++mit){
    errs() << "TS#"<<(*mit).first << " --> ";
...
void GetClassicalCriticalPath::addToTSGates(qGate qg, uint64_t ts)
{
  if(isLeaf){
    //add to tsGates
...
uint64_t GetClassicalCriticalPath::compute_max_ts_of_all_args(qGate qg)
{
  ...
  if(debugGetClassicalCriticalPath){
...
uint64_t GetClassicalCriticalPath::compute_least_slack(Function* F, qGate qg, uint64_t tmax){
...
void GetClassicalCriticalPath::calc_critical_time(Function* F, qGate qg){
    ...
    if(fname.find("llvm.")!=string::npos){ //is intrinsic
      ...
      for(int i=0;i<qg.numArgs; i++){
        ...
        if(argIndex == -1){
          ...
          //update the timestep number for that argument (ajavadia: for multi-qubit gates, only update for target)
          map<int,uint64_t>::iterator indexIter = (*mIter).second.find(argIndex);
          if (i == qg.numArgs-1) {          
...
void GetClassicalCriticalPath::analyzeCallInst(Function* F, Instruction* pInst){
...
void GetClassicalCriticalPath::saveTableFuncQbits(Function* F){
...
void GetClassicalCriticalPath::saveTableFuncQbitsStart(Function* F){
  map<unsigned int, map<int, uint64_t> > tmpFuncQbitsMap;
...
void GetClassicalCriticalPath::init_funcQbitsHalf(uint64_t i){
...
void GetClassicalCriticalPath::gen_half_funcQbits(uint64_t ct, uint64_t hct){
...
void GetClassicalCriticalPath::schedule_alap_insts(uint64_t ct, uint64_t hct){
...
void GetClassicalCriticalPath::process_nonLeafALAP(){
  //copy entries from funcQbits and set values to 1 => zero slack
  init_funcQbitsHalf(1);

}

void GetClassicalCriticalPath::CountCriticalFunctionResources (Function *F) {
...
void GetClassicalCriticalPath::init_gates_as_functions(){
  for(int  i =0; i< NUM_QGATES ; i++){
...
bool GetClassicalCriticalPath::runOnModule (Module &M) {
  init_gate_names();
